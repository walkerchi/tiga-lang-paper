...
Message passing is broader than neighborhood aggregation in a fixed-depth GNN.
...
and optimized sparse execution paths~\cite{pyg}. Graphiler makes the compiler connection explicit:
...
Tiga extends this compiler-oriented programming model with explicit relation
constructors and storage/ownership representations.
...
\midrule
Graphiler & Graph-aware dataflow/residency & Source, edge and destination field roles \\
FeatGraph & Sparse traversal + feature schedules & Joint graph/feature traversal selection \\
Seastar & Vertex-centric fused forward/backward & Message fusion and derivative generation \\
GALA & Composed data/compute and training transforms & Layered representation and transformation scope \\
Ebb / Simit & Relational simulation / hypergraph algebra & Numerical programs beyond GNNs \\
GraphIt & Algorithm/schedule separation & Separate meaning from execution mapping \\
TACO / SparseTIR & Sparse iteration, formats, transformations & Relation traversal separate from storage \\
GraphChi / X-Stream & Disk-oriented partitioned execution & Bounded graph pages and storage traffic \\
Legion & Logical regions and physical placement & Storage instances, ownership and dependencies \\
\bottomrule
\end{tabularx}
