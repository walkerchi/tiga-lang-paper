...
has host-side orchestration; its memory experiment must not be labeled an
end-to-end compiled GPU Storage-IR scheduler. Whole-Tensor LRU eviction is a
...
Reproduce the checked architectural example with
\begin{lstlisting}[language=TigaShell]
gf-opt distributed-plan.mlir -gf-lower-domain-to-iter \
  -gf-lower-iter-to-kernel -gf-select-kernel-schedule \
  -gf-plan-distributed-tasks
\end{lstlisting}
The paper's artifact exporter reruns each stage and rejects a mismatch against
the documentation snapshot before copying the outputs. This validates that the
walkthrough is real compiler output; it does not prove that every ordinary
Python program traverses all these stages.
